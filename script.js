...
try {
    ...
    Object.values(sfx).forEach(sound => {
        if (sound) {
            sound.volume = 0.3;
            // Preload attempt (optional, browsers handle this differently)
            sound.preload = 'auto'; 
        }
    });
} catch (error) {
    console.error("Error initializing Audio objects. Check paths/files. SFX will be disabled.", error);
...
function resizeCanvas() {
    ...
    if (gameArea.offsetHeight > 0) { // If gameArea has a rendered height
        ...
        if(scoreBoardContainer) otherElementsHeight += scoreBoardContainer.offsetHeight + parseInt(getComputedStyle(scoreBoardContainer).marginTop);
        if(pauseButtonContainer) otherElementsHeight += pauseButtonContainer.offsetHeight + parseInt(getComputedStyle(pauseButtonContainer).marginTop);
        if(mobileControls && mobileControls.style.display !== 'none') otherElementsHeight += mobileControls.offsetHeight + parseInt(getComputedStyle(mobileControls).marginTop);
...
if(overlayMenuButton) overlayMenuButton.addEventListener('click', () => { if(gameOverlayMessage) gameOverlayMessage.style.display = 'none'; gameRunning = false; isPaused = false; clearInterval(gameLoopInterval); showScreen(mainMenu); });
...
document.addEventListener('keydown', (e) => { 
    ...
    if(newDirEnum && playerDirection !== newDirEnum){ // Only update if it's a valid new direction
        ...
        // Update currentPlayerDirectionEnum immediately ONLY if game is running and not paused, 
        // to prevent direction change sticking if pressed on menu/pause
        if(gameRunning && !isPaused) {
             // Check if the new direction is not the direct opposite of the current playerDirection (the one snake is moving)
            if (playerDirection.x !== -newDirEnum.x || playerDirection.y !== -newDirEnum.y) {
                 currentPlayerDirectionEnum = newDirEnum;
            }
        } else if (!gameRunning || isPaused) { // If game not running or paused, allow queuing any valid next direction
            currentPlayerDirectionEnum = newDirEnum; // This helps setup initial direction before game starts or resumes
        }
    } 
});
...
function initializeApp() {
    ...
    // Set the GitHub link in credits dynamically if you prefer
